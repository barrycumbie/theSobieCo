...
app.set('views', path.join(__dirname, 'views/adminLogNR'));

// Middleware
app.use(express.static(path.join(__dirname, 'public')));
app.use(express.urlencoded({ extended: true }));
app.use(express.json());
// Opens use of sessions, pull from env
// app.use(session({
...
// Login page
app.get('/loginNR', (req, res) => {
  // Check if user is already logged in
  if (req.session.user) {
    return res.redirect('/dashboardNR');
  }
  res.render('loginNR');
});

// Login process from form
app.post('/loginNR', (req, res) => {
...
// Register page
app.get('/registerNR', (req, res) => {
  // Check if user is already logged in
  if (req.session.user) {
    return res.redirect('/dashboardNR');
  }
  res.render('registerNR');
});

// Register process
app.post('/registerNR', (req, res) => {
  // Grab info from form
...
// Dashboard page
app.get('/dashboardNR', ensureAuthenticated, (req, res) => {
  const db = getDb();
...
// User profile
app.get('/profileNR', ensureAuthenticated, (req, res) => {
  const db = getDb();
  // Get current user data, replace with mongoDB statements
  const user = db.users.find(u => u.id === req.session.user.id);
  res.render('profileNR', { user });
});

// Update profile
app.post('/profileNR', ensureAuthenticated, (req, res) => {
  ...
  res.redirect('/profileNR');
});

// Admin routes
app.get('/adminNR', ensureAuthenticated, ensureAdmin, (req, res) => {
  const db = getDb();
  res.render('adminNR', {
